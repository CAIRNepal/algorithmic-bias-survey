...
Research companion repo for the paper **"FAIR AI Atlas: Empirical Analysis, Systematic Review, and Knowledge Graph Applications in AI Bias Research"** (under revision). Contains analysis scripts for the survey corpus, a metadata extraction notebook, and an interactive Next.js dashboard deployed to GitHub Pages at `https://cairnepal.github.io/algorithmic-bias-survey`.
...
### Primary Corpus
- **`evaluation/analysis/papers_new.csv`** — 519 papers (actively maintained)
...
- **`openalex_enriched.csv`** — metadata fetched via OpenAlex API
  ...
  - **Known abstract issues:** SNs 1, 362, 387 have no abstract (excluded from semantic clustering); SN 67, 304, 451 were manually corrected
  - 516 of 519 papers have valid abstracts
...
**UMAP params (both scripts):** `n_neighbors=15, min_dist=0.0, metric='cosine', random_state=42`
- Grid search confirmed `min_dist=0.0` optimal (silhouette 0.2384)

**HDBSCAN params:** `min_cluster_size=10, min_samples=3, metric='euclidean'`
- Run on mpnet 2D coords only — cluster labels are fixed regardless of which model is displayed
