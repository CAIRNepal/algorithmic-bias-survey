...
Inputs:
- papers_new.csv with at least: "Affiliations" (semicolon-separated) and "Domain".
...
# -----------------------------------------------------------------------------
# TOGGLE
#   FIRST_AUTHOR_ONLY = False  → ALL AUTHORS mode (default)
#   FIRST_AUTHOR_ONLY = True   → FIRST AUTHOR ONLY mode
#
# WHAT IS COUNTED IN EACH MODE?
#   ALL AUTHORS (FIRST_AUTHOR_ONLY=False)
#     • We split the semicolon-separated "Affiliations" for every paper and
#       **explode** them so each author–affiliation becomes one row.
#     • We canonicalize each affiliation to a University via regex mapping.
#     • The heatmap counts **author-affiliation occurrences**:
#         – If a paper has 2 authors from MIT and 1 from Stanford in domain D,
#           MIT contributes +2 and Stanford +1 in D.
#         – Multiple authors from the same university **do** accumulate.
#     • This highlights **people/affiliation capacity** by university.
#
#   FIRST AUTHOR ONLY (FIRST_AUTHOR_ONLY=True)
#     • We take only the **first** affiliation per paper (first author).
#     • Each paper contributes **exactly one** university (after canonicalization).
#     • This highlights **lead/first-author institutions** by university.
#
# WHAT DO THE CELLS SHOW?
#   • Each domain (column) is normalized **column-wise**:
#       percent = (university’s count in domain) / (total count for that domain)
#     across the **displayed universities** (after any Top-N filtering).
#   • Cell annotation is:    "<percent>% (n)"
#       – "<percent>%" = share of the domain credited to that university
#       – "n"           = raw count (author-affiliation rows in ALL AUTHORS mode,
#                         or #papers in FIRST AUTHOR ONLY mode)
#   • The colorbar indicates **percent within domain**; columns will sum to ~100%
#     **over the universities that are shown**. If you limit to Top-N, the
#     column sums may be <100% because hidden universities are omitted.
#
# WHAT ARE THE OUTPUTS?
#   • university_country_domain_cleaned.csv
#       University × Focus Region × Domain with canonicalized University labels
#       and occurrence counts (author-affiliation rows or first-author rows,
#       depending on the mode). Useful if you need the Focus Region dimension.
#   • university_domain_canonical.csv
#       University × Domain counts (this is what the heatmap uses).
#   • unmapped_institutions.csv
#       Pre-cleaned **raw** affiliation strings that didn’t match any canonical
#       regex rule — use this to extend CANONICAL_ORG_MAP.
#
# CANONICALIZATION (why/what)
#   • Affiliations are lightly pre-cleaned (punctuation, whitespace, common
#     trailing country tags) then matched against regex patterns in
#     CANONICAL_ORG_MAP. If no pattern matches, we **fallback to the pre-cleaned
#     string** (stable fallback to avoid drift across runs).
#   • Edit/extend CANONICAL_ORG_MAP to merge variants (e.g., “MIT CSAIL” → “MIT”).
#
# TOP-N FILTERING
#   • TOP_N controls **how many universities** are displayed (by total count).
#     Set TOP_N=None to show **all**.
#   • Domains (columns) that are empty **after** filtering are dropped.
#   • Because percentages are computed after slicing, column sums reflect only
#     the universities kept in view.
#
# FOCUS REGION NOTE
#   • Focus Region is retained in the aggregated CSV (University × Focus Region ×
#     Domain) but not used in the heatmap itself. It’s there for downstream
#     analyses that need the regional dimension.
#
# EDGE CASES & TIPS
#   • Missing/empty "Affiliations" rows are dropped from counting.
#   • If a paper lists the **same university multiple times** (e.g., two authors
#     at the same place), ALL AUTHORS mode legitimately counts both occurrences.
#     If you prefer **unique paper counts** per university/domain instead,
#     re-aggregate with a paper identifier and use nunique() rather than size().
#   • Long domain labels are line-wrapped; figure size adapts to keep text legible.
#
# MINI EXAMPLE (one paper, Domain = "X"):
#   Affiliations:   ["MIT", "MIT", "Stanford"]
#   FIRST_AUTHOR_ONLY=False (ALL AUTHORS):
#       MIT=2, Stanford=1 → column share = MIT 66.7% (2), Stanford 33.3% (1)
#   FIRST_AUTHOR_ONLY=True:
#       Uses only the first affiliation → MIT=1 → MIT 100.0% (1)
# -----------------------------------------------------------------------------
...
FIRST_AUTHOR_ONLY = False   # <- default: ALL AUTHORS
